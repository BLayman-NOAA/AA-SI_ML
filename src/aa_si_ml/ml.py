...
import pandas as pd
import xarray as xr
from echopype.commongrid.utils import xarray_reduce
...
def compute_per_cell_statistics(
        ds_Sv,
        range_bin="2m",
        ping_time_bin="10s",
        statistics=None,
    ...
    they do not conflict with the fine-resolution Sv dimensions.  Pass
    the returned dataset to :func:`add_auxiliary_features` via the
    ``ds_sv`` parameter to absorb the statistics into the ML feature
    matrix.

    Currently supported statistics:

    * ``'cv'`` — coefficient of variation (std / mean) computed in linear
      space per ``(channel, time_bin, range_bin)`` cell.  Stored as
      ``cell_cv``.

    Args:
        ds_Sv (xr.Dataset): Fine-resolution Sv dataset produced by
            ``echopype.calibrate.compute_Sv``.
        range_bin (str): Range bin size matching the downstream
            ``compute_MVBS`` call (e.g. ``"2m"``).
        ping_time_bin (str): Ping-time bin size matching the downstream
            ``compute_MVBS`` call (e.g. ``"10s"``).
        statistics (list[str] or None): Statistics to compute.  Defaults
            to ``['cv']``.
    ...
    Returns:
        ...
        (e.g. ``cell_cv``), each with dims
        ``(channel, cell_ping_time, cell_echo_range)``.

    Raises:
        ValueError: If an unknown statistic name is requested.
    """
    if statistics is None:
        statistics = ['cv']

    unknown = [s for s in statistics if s not in ('cv',)]
    if unknown:
        raise ValueError(
            f"Unknown statistic(s) {unknown}. Supported: 'cv'."
        )
    ...
    ping_breaks = ping_index.union([ping_index[-1] + pd.Timedelta(ping_time_bin)]).values
    ping_interval = pd.IntervalIndex.from_breaks(ping_breaks, closed='left').sort_values()

    computed_stats = {}

    if 'cv' in statistics:
        linear = np.power(10.0, sv_data / 10.0)

        grouped_mean = xarray_reduce(
            linear,
            ds_Sv['channel'],
            ds_Sv['ping_time'],
            ds_Sv['echo_range'],
        ...
        computed_stats['cv'] = xr.where(grouped_mean > 0, grouped_std / grouped_mean, np.nan)

    # --- Store results in ds_out ---
    stat_name_map = {'cv': 'cell_cv'}

    for stat_key, stat_arr in computed_stats.items():
        var_name = stat_name_map[stat_key]
        da = xr.DataArray(
            stat_arr.data,
            dims=['channel', 'cell_ping_time', 'cell_echo_range'],
            coords={
            ...
            attrs={
                ...
                'description': (
                    f'Coefficient of variation computed in linear Sv space over '
                    f'{ping_time_bin} × {range_bin} bins, per channel.'
                ),
                'range_bin': range_bin,
...
def add_auxiliary_features(ds_ml_ready, dataset_name='ml_data_clean', features=None, echodata=None, ds_sv=None):
    ...
    ``'ping_time_seconds'``, ``'seafloor_depth'``, ``'altitude'``, and
    ``'cell_cv'``.  User-defined features are supported via callable dicts.
    ...
    cell_stats_requested = any(
        (f if isinstance(f, str) else f.get('name', '')) in cell_stat_features
        for f in features
    )
    if cell_stats_requested and ds_sv is None:
        raise ValueError(
            "'cell_cv' requires the 'ds_sv' parameter containing output from "
    ...
    for feature_spec in features:
        ...
        if name == 'depth':
            ...
            if ds_sv is None:
                raise ValueError(
                    "'cell_cv' feature requires the 'ds_sv' parameter containing "
                    "output from compute_per_cell_statistics()."
                )
            if 'cell_cv' not in ds_sv:
                raise ValueError(
                    "'cell_cv' variable not found in ds_sv. "
                    "Ensure compute_per_cell_statistics() was called with statistics=['cv']."
                )
            stat_da = ds_sv['cell_cv']  # (channel, cell_ping_time, cell_echo_range)
            # Average across channels → 2D (cell_ping_time, cell_echo_range)
            stat_2d = stat_da.mean(dim='channel')
            stat_2d_t = stat_2d.transpose('cell_ping_time', 'cell_echo_range')

            # Fast path: shapes match — use integer indexing directly
            if stat_2d_t.shape == grid_shape:
                values = stat_2d_t.values[ping_indices, range_indices]
            else:
                # Fallback: coordinate-based nearest-neighbour lookup
                ml_ping_times = ds_ml_ready[ping_dim].values[ping_indices]
                ml_ranges = ds_ml_ready[range_dim].values[range_indices]
                # Build a DataArray of per-sample values via vectorised sel
                ping_da = xr.DataArray(ml_ping_times, dims='sample')
                range_da = xr.DataArray(ml_ranges, dims='sample')
                values = stat_2d_t.sel(
                    cell_ping_time=ping_da,
                    cell_echo_range=range_da,
                    method='nearest',
                ).values
        ...
        if nan_count > 0:
            fill_val = np.nanmedian(values)
            if np.isnan(fill_val):
                fill_val = 0.0
            values = np.where(np.isnan(values), fill_val, values)
            logger.warning(
                "Feature '%s': filled %d NaN value(s) with median (%.4f).",
                name, nan_count, fill_val,
            )
        new_columns.append((name, values))
...
def run_hdbscan(
    ...
    if find_background_cluster:
        dbscan_results, background_label = retrieve_background_cluster(
            X,
            sample_indices,
            min_samples,
            sample_size,
    ...
    clustering_result = dict(dbscan_results[first_key])
    clustering_result.update(
        {
            ...
            'plot_hierarchy': use_hdbscan and not find_background_cluster,
        }
    )
...
def _embed_single_clustering_result(
    ...
    if 'labels' not in clustering_result or 'sample_indices' not in clustering_result:
        raise ValueError("clustering_result must contain 'labels' and 'sample_indices'")

    ds_final = store_ml_results_flattened(
        ds_normalized,
        clustering_result['labels'],
        ml_result_name,
        dataset_name=dataset_name,
        result_sample_indices=clustering_result['sample_indices'],
    )

    gridded_results = extract_ml_data_gridded(
        ds_final,
        ml_result_name,
        dataset_name=dataset_name,
        fill_value=np.nan,
        store_in_dataset=True,
    )

    return {
        'ds_normalized': ds_final,
        'gridded_results': gridded_results,
    }


def _plot_single_clustering_result(
        ds_normalized,
        clustering_result,
        dataset_name,
        ml_result_name,
    ...
    echogram.plot_cluster_echogram(
        ...
        max_depth=resolved_plot_window[1],
        ping_min=resolved_plot_window[2],
        ping_max=resolved_plot_window[3],
...
def embed_clustering_results(
        ds_normalized,
        clustering_results,
        dataset_name,
        ml_result_name,
        ):
    """Embed one or more clustering results into the normalized dataset."""
    if isinstance(clustering_results, list):
        ds_final = ds_normalized
        gridded_results = {}
        ...
        for index, clustering_result in enumerate(clustering_results, start=1):
            ...
            ds_final = embedded['ds_normalized']
            gridded_results[result_name] = embedded['gridded_results']
...
def extract_data_and_run_hdbscan(
    ...
    embedding_output = embed_clustering_results(
        ds_normalized,
    ...
    plot_clustering_report(
        embedding_output['ds_normalized'],
        clustering_output['clustering_results'],
        dataset_name=custom_dataset_name,
        ml_result_name=ml_result_name,
    ...
    if find_background_cluster:
        return (
            embedding_output['ds_normalized'],
            embedding_output['gridded_results'],
            clustering_output['dbscan_results'],
            clustering_output['background_label'],
        )
    return (
        embedding_output['ds_normalized'],
        embedding_output['gridded_results'],
